            ...
            try:
                ...
                if "CopyBone_arr_base" in fdk_config_json_data:
                    if "change_tail" in fdk_config_json_data["CopyBone_arr_base"]:
                        context.scene.fdk_opt_change_tail = fdk_config_json_data["CopyBone_arr_base"]["change_tail"]
                        self.report({'INFO'}, f"change_tail={context.scene.fdk_opt_change_tail}")
                    if "change_matrix" in fdk_config_json_data["CopyBone_arr_base"]:
                        context.scene.fdk_opt_change_matrix = fdk_config_json_data["CopyBone_arr_base"]["change_matrix"]
                        self.report({'INFO'}, f"change_matrix={context.scene.fdk_opt_change_matrix}")
                    if "reset_empty" in fdk_config_json_data["CopyBone_arr_base"]:
                        context.scene.fdk_opt_reset_empty = fdk_config_json_data["CopyBone_arr_base"]["reset_empty"]
                        self.report({'INFO'}, f"reset_empty={context.scene.fdk_opt_reset_empty}")
                warning = ""
    ...
    def execute(self, context):
        if bpy.context.scene.fdk_config_json_data is None:
            ...
            try:
                ...
                arr_add=[]
        try:
            changematrix=context.scene["fdk_opt_change_matrix"]
        except:
            context.scene["fdk_opt_change_matrix"]=False
        try:
            resetempty=context.scene["fdk_opt_reset_empty"]
        except:
            context.scene["fdk_opt_reset_empty"]=True
        try:
            changetail=context.scene["fdk_opt_change_tail"]
        except:
            context.scene["fdk_opt_change_tail"]=True
...
class FDK_PT_Snippets(bpy.types.Panel):
    ...
    def poll(cls, context):
        return context.scene.active_xbone_subpanel == 'Common'
    ...
    def draw(self, context):
        ...
        # row.prop(context.scene, "fdk_opt_change_matrix", text="copy_matrix")
        # row.prop(context.scene, "fdk_opt_change_tail", text="copy_tail")
        # row.prop(context.scene, "fdk_opt_reset_empty", text="reset_empty")
...
def register():
    ...
    # )
    bpy.types.Scene.fdk_opt_change_matrix = bpy.props.BoolProperty(
        name="copy matrix",description="(实验功能)",default= False
    )
    bpy.types.Scene.fdk_opt_change_tail = bpy.props.BoolProperty(
        name="copy tail",description="(实验功能)",default= True
    )
    bpy.types.Scene.fdk_opt_reset_empty = bpy.props.BoolProperty(
...
def unregister():
    ...
    del bpy.types.Scene.fdk_opt_change_matrix
    del bpy.types.Scene.fdk_opt_change_tail
    del bpy.types.Scene.fdk_opt_reset_empty
